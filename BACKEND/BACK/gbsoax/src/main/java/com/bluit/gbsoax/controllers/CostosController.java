...
public class CostosController {
    ...
    @GetMapping(path = "/{id}")
    public Optional<CostosModel> obtenerPorid(@PathVariable("id") Long id){
        return this.costosServices.obtenerPorId(id);
    }

    @DeleteMapping(path = "/{id}")
    public String eliminarUsuario(@PathVariable("id") long id){
        boolean ok = this.costosServices.eliminarCosto(id);
        if(ok){
            return "eliminado" + id;
        }else{
